...
for i,file in enumerate(file_paths):
    ...
    unique_indices = sets[-1].index_set()
    nr_unique_indices = len(unique_indices)
    nr = sets[i].size()
...
for i,s in enumerate(sets):
    ...
    axes[0].scatter(x,y,s=10,facecolors='none',edgecolors=colors[i],label=names[-1])
...
axes[1].hist(a,bins=np.linspace(0,50,25), color=colors, label=names)
